...
import (
	...
	gen "github.com/deweing/protoc-gen-openapiv3/internal/generator"
...
func mergeTargetFile(targets []*wrapper, mergeFileName string) *wrapper {
	...
	for _, f := range targets {
		if mergedTarget == nil {
			...
			for k, v := range f.swagger.Components.SecuritySchemes {
				mergedTarget.swagger.Components.SecuritySchemes[k] = v
			}
...
func (g *generator) Generate(targets []*descriptor.File) ([]*descriptor.ResponseFile, error) {
	...
	if g.reg.IsAllowMerge() {
		...
		for _, f := range targets {
			if proto.HasExtension(f.Options, openapi_options.E_Swagger) {
				mergedTarget = f
				break
			}
		}
